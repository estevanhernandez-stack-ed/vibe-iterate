...
**Tech Stack:** Markdown + YAML frontmatter (SKILL files), JSON Schema Draft 2020-12 (state files), `ajv-cli` (CLI schema validation, installed via `npx`), bash (test scripts).
...
run_validate() {
  ...
  if npx --yes ajv-cli@5 validate -s "$schema" -d "$data" --spec=draft2020 --all-errors >/dev/null 2>&1; then
